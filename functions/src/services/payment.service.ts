...
export class PaymentService extends BaseService {
   ...
   * Captures locked_job_value at this moment (immutable)
   */
  async initializePayment(data: {
    ...
    try {
      this.validateRequired(data, ['email', 'amount', 'clientUid', 'type']);
      ...
      // Calculate commission if escrow
      let commissionRetained = 0;
      let lockedJobValue = 0;

      if (data.type === 'escrow') {
        lockedJobValue = data.amount;
        commissionRetained = lockedJobValue * 0.10; // 10% commission
      }

      // Create transaction record
      await this.db.collection(COLLECTIONS.TRANSACTIONS).add({
        job_id: data.jobId,
        match_id: data.matchId,
        client_uid: data.clientUid,
        artisan_uid: data.artisanUid,
        ...
        locked_job_value: lockedJobValue || null,
        commission_retained: commissionRetained || null,
        metadata: data.metadata,
        created_at: admin.firestore.FieldValue.serverTimestamp(),
        updated_at: admin.firestore.FieldValue.serverTimestamp()
      });

      this.logOperation('payment-initialized', { reference, type: data.type, amount: data.amount });
      ...
      this.logger.error('Payment initialization failed', { error: error.message });
